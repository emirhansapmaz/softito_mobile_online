// SoftITO Mobil App Developer - Ödev 2: Orijinal Hatalı Kod

// 1. LSP İHLALİ: DijitalUrun, Urun sınıfının kargoUcretiHesapla metodunu bozuyor (Exception fırlatıyor).
class Urun {
  String id;
  String ad;
  double fiyat;
...
class DijitalUrun extends Urun {
  ...
  @override
  double kargoUcretiHesapla() {
    // LSP İhlali: Üst sınıfın davranışı bozuluyor, kod patlıyor!
    throw Exception("Dijital urunlerde kargo hesaplanamaz!");
  }
}

// 2. ISP İHLALİ: ISiparisIslemleri çok fazla sorumluluğu tek bir interface'e yüklemiş (Fat Interface).
...
// 3. SRP, DIP ve OCP İHLALLERİ:
// - SRP İhlali: Bu sınıf ödeme, kargo, fatura, indirim, mail ve sms gibi her şeyi kendisi yapıyor.
// - DIP İhlali: Doğrudan Sqlite, SmtpMail ve Netgsm somut sınıflarını new'leyerek sıkı bağımlılık oluşturuyor.
// - OCP İhlali: odemeYap ve kupon kontrolleri if-else ile yapılmış; yeni yöntem eklemek için kodu değiştirmek gerekiyor.
class SiparisYoneticisi implements ISiparisIslemleri {
  ...
  void siparisTamamla(
    ...
    for (var i = 0; i < sepet.length; i++) {
      ...
      toplam += sepet[i].kargoUcretiHesapla(); // Dijital üründe hata fırlatıp programı çökertecek!
      sepet[i].stok--;
    }
